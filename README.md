...
> Status: **v0.1.0-alpha.1 — E2E tested locally**. Pairing + chat + persistence + i18n work between two nodes. Not audited. **Not yet safe for life-safety scenarios** (journalists, activists under threat) — see "Security" below for the current threat model.
...
Full threat model and caveats for journalists / activists will land in `SECURITY.md` before v0.2. File an issue if you are considering using demi-node under threat — do not deploy v0.1.x alpha for life-safety scenarios.
